...
struct LasStandardData : public Resource
{
    ...
    void loadHeader()
    {
        ...
        data->numPoints = min(data->numPoints, (int64_t)1'000'000'000ll);
    ...
    void process(Renderer *renderer)
    {
        ...
        if (this->task)
        {
            glNamedBufferSubData(this->ssPoints.handle, 16 * numPointsLoaded, this->task->buffer->size,
                                 this->task->buffer->data);
