...
#define MASK_10BIT 1023
    ...
    struct Node
    {
        ...
        shared_ptr<Node> children[8] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};
    };
    ...
    void loadMetadata()
    {
        ...
        for (int i = 0; i < jsAttributes.size(); i++)
        {
            ...
            bytesPerPoint += jsAttribute["size"].get<int64_t>();
        }
    ...
    void loadHierarchy()
    {
        ...
        traverse(root.get(), [&nodes](Node *node) { nodes.push_back(*node); });

        std::sort(std::execution::par_unseq, nodes.begin(), nodes.end(),
                  [](Node a, Node b) { return a.byteOffset < b.byteOffset; });
        ...
        std::sort(bins.begin(), bins.end(), [](shared_ptr<Bin> a, shared_ptr<Bin> b) { return a->weight < b->weight; });
    ...
    void process(Renderer *renderer)
    {
        ...
        if (this->task)
        {
            ...
            glNamedBufferSubData(this->ssXyz_12b.handle, 4 * task->pointOffset, task->buffer_12b->size,
                                 task->buffer_12b->data);

            glNamedBufferSubData(this->ssXyz_8b.handle, 4 * task->pointOffset, task->buffer_8b->size,
                                 task->buffer_12b->data);

            glNamedBufferSubData(this->ssXyz_4b.handle, 4 * task->pointOffset, task->buffer_4b->size,
                                 task->buffer_4b->data);

            glNamedBufferSubData(this->ssColors.handle, 4 * task->pointOffset, task->buffer_colors->size,
