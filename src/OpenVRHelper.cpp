...
{
    int rightHandID =
        system->GetTrackedDeviceIndexForControllerRole(vr::ETrackedControllerRole::TrackedControllerRole_RightHand);
...
//-----------------------------------------------------------------------------
string OpenVRHelper::getTrackedDeviceString(TrackedDeviceIndex_t device, TrackedDeviceProperty prop,
...
dmat4 OpenVRHelper::getProjection(EVREye eye, float nearClip, float farClip)
{
    ...
    auto customProj = glm::dmat4(2.0 * idx, 0.0, 0.0, 0.0, 0.0, 2.0 * idy, 0.0, 0.0, sx * idx, sy * idy, 0.0, -1.0, 0.0,
                                 0.0, zNear, 0.0);
...
dmat4 OpenVRHelper::getEyePose(Hmd_Eye nEye)
{
    ...
    dmat4 matrix(mat.m[0][0], mat.m[1][0], mat.m[2][0], 0.0, mat.m[0][1], mat.m[1][1], mat.m[2][1], 0.0, mat.m[0][2],
...
dmat4 OpenVRHelper::steamToGLM(const HmdMatrix34_t &mSteam)
{
    ...
    dmat4 matrix{
        mSteam.m[0][0], mSteam.m[1][0], mSteam.m[2][0], 0.0, mSteam.m[0][1], mSteam.m[1][1], mSteam.m[2][1], 0.0,
        mSteam.m[0][2], mSteam.m[1][2], mSteam.m[2][2], 0.0, mSteam.m[0][3], mSteam.m[1][3], mSteam.m[2][3], 1.0f};
